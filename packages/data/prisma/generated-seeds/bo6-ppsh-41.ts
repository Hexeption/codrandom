...
export async function seed_bO6_pPSh41(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "PPSh-41", category: "submachine gun", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "7.62x25mm Tokarev Overpressured", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Recoil Springs", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "7.62x25mm Tokarev FMJ", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
}
