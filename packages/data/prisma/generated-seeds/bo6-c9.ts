...
export async function seed_bO6_c9(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "C9", category: "submachine gun", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "C9 10mm Auto 30-Round Mags", "magazine"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "9x19mm Parabellum Overpressured", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Recoil Springs", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "9x19mm Parabellum FMJ", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "10mm Overpressured", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "10mm FMJ", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
}
