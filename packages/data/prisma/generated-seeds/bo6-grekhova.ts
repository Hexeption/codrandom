...
export async function seed_bO6_grekhova(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Grekhova", category: "secondary", game: "BO6", primary: false, secondary: true })
  ...
  { const att = await ensureAttachment(prisma, "Weighted Stock", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Akimbo Grekhova", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
