...
export async function seed_bO6_gS45(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "GS45", category: "secondary", game: "BO6", primary: false, secondary: true })
  ...
  { const att = await ensureAttachment(prisma, "Akimbo GS45", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, ".45 ACP Overpressured", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Recoil Springs", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, ".45 ACP FMJ", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
}
