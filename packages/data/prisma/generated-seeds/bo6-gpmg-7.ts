...
export async function seed_bO6_gPMG7(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "GPMG-7", category: "light machine gun", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Extended Mag IV", "magazine"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
