...
export async function seed_bO6_xM4(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "XM4", category: "assault rifle", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "XM4 3-Round Burst Mod", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
