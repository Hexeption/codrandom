...
export async function seed_bO6_tR2(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "TR2", category: "marksman rifle", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "TR2 CQB Auto Conversion", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
}
