...
export async function seed_bO6_goblinMk2(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Goblin Mk2", category: "assault rifle", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Goblin Mk2 7.62 Mini-rocket Conversion", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
