...
export async function seed_bO6_tanto22(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Tanto .22", category: "submachine gun", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Tanto .22 Binary Trigger", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
