...
export async function seed_bO6_9mmPM(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "9mm PM", category: "secondary", game: "BO6", primary: false, secondary: true })
  ...
  { const att = await ensureAttachment(prisma, "Kepler Pistol Scope", "optic"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "9MM PM Skull Splitter Barrel", "barrel"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Skull Splitter Extended Mag", "magazine"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Akimbo 9MM PM", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "9x18mm Makarov Overpressured", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Recoil Springs", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "9x18mm Makarov FMJ", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
}
