...
export async function seed_bO6_stryder22(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Stryder .22", category: "pistol", game: "BO6", primary: false, secondary: true })
  ...
  { const att = await ensureAttachment(prisma, "Akimbo Stryder .22", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, ".22 WMR Overpressured", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Recoil Springs", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, ".22 WMR FMJ", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Stryder .22 3-Round Burst Mod", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
}
