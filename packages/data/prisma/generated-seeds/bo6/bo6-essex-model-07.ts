...
export async function seed_bO6_essexModel07(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Essex Model 07", category: "marksman rifle", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Stripper Clip", "magazine"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Extended Mag II", "magazine"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Quickdraw Lever", "lever"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Assault Lever", "lever"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Commando Lever", "lever"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Ergonomic Lever", "lever"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "CQB Lever", "lever"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Cut-Off Stock", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
