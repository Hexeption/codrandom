...
export async function seed_bO6_saug(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Saug", category: "submachine gun", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Assult Grip", "rear_grip"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Akimbo Saug", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
