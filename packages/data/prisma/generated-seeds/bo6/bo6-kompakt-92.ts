...
export async function seed_bO6_kompakt92(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Kompakt 92", category: "submachine gun", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Integrated Suppressor Barrel", "barrel"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Kompakt 92 3-Round Burst Mod", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
