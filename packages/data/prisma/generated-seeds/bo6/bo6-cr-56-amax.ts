...
export async function seed_bO6_cR56AMAX(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "CR-56 AMAX", category: "assault rifle", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "M-67 10-R Mags", "magazine"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
