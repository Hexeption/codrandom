...
export async function seed_bO6_pP919(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "PP-919", category: "submachine gun", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Fast Mag III", "magazine"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
