...
export async function seed_mW2_kastov545(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Kastov 545", category: "assault rifle", game: "MW2", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Kastovia 406", "barrel"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "JAK Requiem", "rear_grip"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Broadside FCT", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
