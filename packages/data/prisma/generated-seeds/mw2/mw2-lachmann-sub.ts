...
export async function seed_mW2_lachmannSub(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "Lachmann Sub", category: "submachine gun", game: "MW2", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "FTAC M-SUB 12\"", "barrel"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "Lachmann Pulsar", "barrel"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "L38 Falcon 226mm", "barrel"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Lachmann S76 Factory Stock", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "LM Stockless Mod", "stock"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "9mm Hollow Point", "ammo"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
