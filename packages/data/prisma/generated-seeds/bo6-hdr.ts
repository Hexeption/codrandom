...
export async function seed_bO6_hDR(prisma: PrismaClient) {
  const weapon = await ensureWeapon(prisma, { name: "HDR", category: "sniper rifle", game: "BO6", primary: true, secondary: false })
  ...
  { const att = await ensureAttachment(prisma, "Lightweight Bipod", "underbarrel"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Marksman Bipod", "underbarrel"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "Extended Mags II", "magazine"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  ...
  { const att = await ensureAttachment(prisma, "108mm Overpressured", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
  { const att = await ensureAttachment(prisma, "108mm FMJ", "fire_mod"); await ensureWeaponAttachment(prisma, weapon.id, att.id) }
